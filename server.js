...
function playRadioStation(radioStation) {
    ...
    radio.trackObject.track.numCurrentSegment = 0;
		...
		console.log(
			`⏭️ | ${radio.name} - Playing next track (Track #${radio.trackNum})`
		);
	...
	https
		.get(request.body.downloadURL, (response) => {
			if (response.statusCode !== 200) {
				console.error(
					`Error fetching MP3 from URL: ${response.statusCode}`
				);
				process.exit(1);
			}
			...
			response.on("data", (chunk) => {
				...
				while (chunkData.length >= chunkSize) {
					...
					console.log(
						`making file: chunks/chunk-${currentChunk + 1}.mp3`
					);
					fs.writeFileSync(chunkFilename, chunkBuffer);
					console.log(
						`writing file: chunks/chunk-${currentChunk + 1}.mp3`
					);
					uploadMP3ToFirebase(
						chunkFilename,
						`Tracks/${request.body.title}/Chunk_${
							currentChunk - 1
						}.mp3`,
						(data) => {
							// access the duration of the temporary file
							const duration = mp3Duration(chunkFilename).then(
								(data) => {
									// console.log(data);
									trackChunkDurationArray[
										trackChunkDurationArray.length
									] = data;
					...
					console.log(
						`Chunk ${currentChunk - 1} saved to: ${chunkFilename}`
					);
				}
			});
					...
					uploadMP3ToFirebase(
						...
						(data) => {
							...
							deleteStorageFile(
								...
								console.log(
									"🚮 | Deleted source MP3 successfully"
								)
							);

							uploadTrackRefToDatabase(
								request,
								chunkMediaDurationArray,
								chunkMediaDurationArray.length - 1
							);
						}
					);
					console.log(
